...
exports.RimEdgesGeometry = void 0;
 ...
 */
class RimEdgesGeometry extends three_1.BufferGeometry {
    constructor(geometry = null, thresholdAngle = 1) {
        ...
        const indexedPositions = RimEdgesGeometry.convertAttributeToVector3Array(geometry2.attributes.position);
        const faceNormalInfos = RimEdgesGeometry.getFaceNormalInfo(geometry2);
        const edgeData = RimEdgesGeometry.extractEdgeDataMap(faceNormalInfos);
        // generate vertices
        const v3 = new three_1.Vector3();
        const n = new three_1.Vector3();
        const n1 = new three_1.Vector3();
        const n2 = new three_1.Vector3();
        const d = new three_1.Vector3();
        const thresholdDot = Math.cos(three_1.MathUtils.DEG2RAD * thresholdAngle);
        for (let key in edgeData) {
            ...
            if (
            // edge.face2 == null ||
            faceNormalInfos[edge.face1].normal.dot(faceNormalInfos[edge.face2].normal) <= thresholdDot) {
        ...
        // build geometry
        this.setAttribute("position", new three_1.Float32BufferAttribute(vertices, 3, false));
        this.setAttribute("control0", new three_1.Float32BufferAttribute(control0, 3, false));
        this.setAttribute("control1", new three_1.Float32BufferAttribute(control1, 3, false));
        this.setAttribute("direction", new three_1.Float32BufferAttribute(direction, 3, false));
        this.setAttribute("collapse", new three_1.Float32BufferAttribute(collapse, 1, false));
    }
    static margeGeometry(geometry) {
        const geometry2 = (0, BufferGeometryUtils_1.mergeVertices)(geometry.clone());
     ...
     * @private
     */
    static convertAttributeToVector3Array(attribute) {
        const array = [];
        for (let s = 0; s < attribute.array.length; s++) {
            array.push(new three_1.Vector3(attribute.array[s * 3], attribute.array[s * 3 + 1], attribute.array[s * 3 + 2]));
        }
        return array;
    }
    /**
     ...
     * @private
     */
    static getFaceNormalInfo(geometry) {
        const index = geometry.index;
        const faceCount = index.count / 3;
        const sourceVertices = geometry.attributes.position;
        const faceNormalInfos = [];
        const _triangle = new three_1.Triangle();
        const a = new three_1.Vector3();
        const b = new three_1.Vector3();
        const c = new three_1.Vector3();
        for (let i = 0; i < faceCount; i++) {
            ...
            const faceNormalInfo = {
                ...
                b: bIndex,
                c: cIndex,
                normal: _triangle.getNormal(new three_1.Vector3()),
            };
            faceNormalInfos.push(faceNormalInfo);
        }
        return faceNormalInfos;
    }
    static extractEdgeDataMap(faceNormalInfos) {
        const keys = ["a", "b", "c"];
...
exports.RimEdgesGeometry = RimEdgesGeometry;
